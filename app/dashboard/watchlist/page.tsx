...
export default function WatchlistBentoGrid() {
    ...
    return (
        // 🌟 ADAPTIVE CANVAS VIEWPORT: Balanced padding for seamless mobile rendering
        <div className="min-h-screen bg-zinc-950 text-zinc-50 p-2 sm:p-4 md:p-6 space-y-6 antialiased selection:bg-violet-500/30">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-zinc-900 pb-5 gap-4">
                ...
                <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap justify-between sm:justify-end w-full sm:w-auto">
                    {/* 🌟 SCROLLABLE TABS ON MOBILE: Prevents broken wrapped tracking tabs on small mobile rows */}
                    <div className="flex items-center gap-1 bg-zinc-900/60 border border-zinc-900 p-1 rounded-xl max-w-full overflow-x-auto scrollbar-none shrink-0">
                        {["ALL", "METALS", "ENERGY", "AGRICULTURE"].map((cat) => (
                            <button
                                key={cat}
                                onClick={() => setFilterCategory(cat)}
                                className={`text-xs font-medium px-3 sm:px-3.5 py-1.5 rounded-lg whitespace-nowrap transition-all duration-200 ${
                                    filterCategory === cat
                    ...
                    <button
                        ...
                        className="p-2.5 bg-zinc-900/40 hover:bg-zinc-900 border border-zinc-900 hover:border-zinc-800 rounded-xl text-zinc-400 hover:text-zinc-200 transition shrink-0"
            ...
            {/* 🌟 FIX: Removed fixed auto-rows constraint on mobile and built multi-breakpoint structures */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 auto-rows-auto md:auto-rows-[190px]">

                {/* COMPONENT A: GOLD MACRO BOX */}
                {goldAsset && (
                    <div className="md:col-span-2 md:row-span-2 bg-gradient-to-b from-zinc-900/40 to-zinc-900/10 border border-zinc-900 rounded-3xl p-5 sm:p-6 flex flex-col justify-between shadow-xl relative overflow-hidden group hover:border-zinc-800/80 transition-all duration-300 min-h-[220px] md:min-h-full">
                        ...
                        {/* 🌟 MOBILE STACK FIX: Stacks metrics vs graphics on small mobile blocks, side-by-side on tablet/desktop */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end z-10 mt-6 md:mt-0">
                            <div className="space-y-1">
                                <span className="text-xs font-medium text-zinc-500 block">Database Spot Price</span>
                                <span className="text-2xl sm:text-3xl font-bold font-mono tracking-tight text-zinc-50">${goldAsset.currentPrice.toLocaleString()}</span>
                ...
                {energyAsset && (
                    <div className="md:col-span-1 md:row-span-2 bg-zinc-900/20 border border-zinc-900 rounded-3xl p-5 flex flex-col justify-between hover:border-zinc-800 transition-all duration-200 shadow-md group min-h-[200px] md:min-h-full">
                ...
                {/* COMPONENT C: ANALYTICS SYNC METRIC */}
                <div className="md:col-span-1 md:row-span-2 bg-zinc-900/40 border border-zinc-900 border-dashed rounded-3xl p-5 flex flex-col justify-between min-h-[220px] md:min-h-full">
                    ...
                    <div className="pt-4 mt-4 md:mt-0 border-t border-zinc-900 text-[11px] text-zinc-500 flex items-center gap-2 font-mono">
                ...
                {filteredItems.map((asset) => {
                    ...
                    return (
                        // 🌟 DYNAMIC ROW HEIGHT: Card self-adjusts gracefully on grid rearrangements
                        <div key={asset.id} className="bg-zinc-900/20 border border-zinc-900 rounded-2xl p-4 flex flex-col justify-between hover:border-zinc-800/80 transition shadow-sm relative group min-h-[110px] md:min-h-full">
                            <div className="flex items-center justify-between">
                                <div className="truncate mr-2">
                                    ...
                                    <span className="text-[10px] text-zinc-500 ml-2 font-medium hidden sm:inline-block truncate max-w-[80px]">
                                        {asset.category}
                                    </span>
                                </div>
                                <button
                                    onClick={() => removeAsset(asset.id)}
                                    // 🌟 MOBILE ACCESSIBILITY: Keeps button interaction visible on touch surfaces where hover states don't exist
                                    className="text-zinc-600 lg:text-zinc-700 hover:text-rose-400 transition p-1 lg:opacity-0 lg:group-hover:opacity-100 duration-150"
                                >
                                    <EyeOff className="h-3.5 w-3.5 lg:h-3 lg:w-3" />
                                </button>
                            </div>
                            ...
                            <div className="flex items-end justify-between mt-4 md:mt-2">
                                ...
                                <div className="h-8 w-16 opacity-80 lg:opacity-70 lg:group-hover:opacity-100 transition duration-200">
                                    <ResponsiveContainer width="100%" height="100%">
