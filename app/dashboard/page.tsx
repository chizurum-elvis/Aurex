...
export default function DashboardPage() {
    ...
    // Fetch live data directly from your verified Atlas API route
    useEffect(() => {
        async function fetchPrices() {
            try {
                ...
                if (data.commodities) {
                    // Injected percentage changes for high-fidelity visual layout dynamics
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-50 p-6 space-y-6 antialiased">
          ...
          <span className="text-xs text-zinc-400 bg-zinc-900 px-3 py-1.5 rounded-md border border-zinc-800 flex items-center gap-2">
            ...
            <div className="w-full bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 overflow-hidden shadow-inner backdrop-blur-sm">
                <div className="flex items-center gap-8 animate-marquee whitespace-nowrap">
                    {commodities.map((item) => (
                        <div key={`ticker-${item._id}`} className="inline-flex items-center gap-2 text-sm border-r border-zinc-800 pr-8">
                ...
                {(item.change24h || 0) >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-zinc-900/20 border border-zinc-900 p-4 rounded-xl flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-zinc-500 uppercase">Tracked Assets</p>
                ...
                <div className="bg-zinc-900/20 border border-zinc-900 p-4 rounded-xl flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-zinc-500 uppercase">Top Gainer</p>
                ...
                <div className="bg-zinc-900/20 border border-zinc-900 p-4 rounded-xl flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-zinc-500 uppercase">Watchlist Size</p>
                        <h3 className="text-2xl font-bold font-mono text-violet-400 mt-1">4 Pins</h3>
                    </div>
                    <Star className="h-8 w-8 text-violet-500/40" />
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-zinc-900/30 p-3 rounded-xl border border-zinc-900">
                    <div className="relative w-full sm:w-80">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-950 text-sm pl-10 pr-4 py-2 rounded-lg border border-zinc-800 text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
                        ...
                        {["All", "Metals", "Energy", "Agriculture"].map((category) => (
                            <button
                                ...
                                className={`text-xs font-medium px-4 py-2 rounded-lg border transition whitespace-nowrap ${
                ...
                <div className="bg-zinc-900/10 border border-zinc-900 rounded-xl overflow-hidden shadow-2xl backdrop-blur-md">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tr className="border-b border-zinc-900 bg-zinc-900/40 text-zinc-500 text-xs font-semibold tracking-wider uppercase">
                                ...
                                <th className="py-4 px-6">Region Origin</th>
                                <th className="py-4 px-6 text-right">Live Index Value</th>
                                <th className="py-4 px-6 text-right">24h Variance</th>
                            </tr>
                            </thead>
                            ...
                            {loading ? (
                                [...Array(4)].map((_, i) => (
                                    <tr key={`skeleton-${i}`} className="animate-pulse">
                                        <td colSpan={5} className="py-6 px-6 h-12 bg-zinc-900/5" />
                                    </tr>
                                ))
                                ...
                                commodities.map((item) => (
                                    ...
                                    >
                                        <td className="py-4 px-6 font-semibold text-zinc-200 group-hover:text-violet-400 transition-colors">
                                            {item.name}
                                        </td>
                                        <td className="py-4 px-6">
                        <span className="inline-flex items-center gap-1 text-xs bg-zinc-900/80 border border-zinc-800 text-zinc-300 px-2.5 py-1 rounded-md">
                          <Layers className="h-3 w-3 text-violet-500" />
                            {item.category}
                                        ...
                                        <td className="py-4 px-6 text-right font-mono font-bold text-zinc-100">
                                            ...
                                            <span className="text-xs text-zinc-500 font-normal block mt-0.5">per {item.unit || "Unit"}</span>
                                        </td>
