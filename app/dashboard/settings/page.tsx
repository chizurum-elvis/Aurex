...
export default function SettingsPage() {
    ...
    return (
        <form onSubmit={handleSaveChanges} className="min-h-screen bg-zinc-950 text-zinc-100 p-4 sm:p-6 space-y-6 antialiased selection:bg-violet-500/30">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-zinc-900 pb-5 gap-4">
                ...
                <button
                    type="submit"
                    disabled={isSaving}
                    className="flex items-center justify-center gap-2 w-full sm:w-auto px-4 py-2.5 sm:py-2 bg-zinc-50 hover:bg-zinc-200 disabled:bg-zinc-800 disabled:text-zinc-600 text-zinc-950 text-xs font-semibold rounded-xl transition duration-150 active:scale-98 shrink-0"
                >
                    {saveSuccess ? (
                        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8 items-start">
                ...
                <div className="flex flex-row md:flex-col overflow-x-auto md:overflow-visible pb-2 md:pb-0 gap-1 md:sticky md:top-6 border-b border-zinc-900/60 md:border-none scrollbar-none shrink-0">
                    {sections.map((tab) => {
                        const Icon = tab.icon;
                        const isSelected = activeTab === tab.id;
                        return (
                            <button
                                // 🌟 FIXED THE TYPO HERE: Corrected `key={tab.id;` to `key={tab.id}`
                                key={tab.id}
                                type="button"
                                onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all border whitespace-nowrap ${
                                    isSelected
                            ...
                            >
                                <Icon className={`h-4 w-4 shrink-0 ${isSelected ? "text-violet-400" : "text-zinc-600"}`} />
                                {tab.label}
                            </button>
                        );
                    })}
                ...
                <div className="md:col-span-3 space-y-6 max-w-2xl w-full">

                    {/* TAB CONTENT MODULE 1: PROFILE MANAGEMENT */}
                    {activeTab === "general" && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
                            <div className="bg-zinc-900/10 border border-zinc-900 rounded-2xl p-5 sm:p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] sm:text-[11px] font-mono font-semibold text-zinc-500 uppercase tracking-wider">
                                            Display Name
                                        </label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-zinc-950 border border-zinc-900 focus:border-zinc-800 focus:outline-none rounded-xl p-3 text-xs font-medium font-mono tracking-tight text-zinc-500 opacity-60 cursor-not-allowed truncate"
                                            disabled
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="bg-zinc-900/10 border border-zinc-900 rounded-2xl p-5 sm:p-6 space-y-4">
                                ...
                                <div className="flex items-start sm:items-center justify-between gap-4 p-1">
                                    <div className="space-y-0.5">
                                        <div className="text-xs font-medium text-zinc-200 flex items-center gap-2">
                                            <Bell className="h-3.5 w-3.5 text-zinc-500 shrink-0" />
                                    ...
                                    <input
                                        ...
                                        className="h-4 w-4 mt-0.5 sm:mt-0 rounded border-zinc-900 bg-zinc-950 text-violet-500 focus:ring-0 focus:ring-offset-0 cursor-pointer accent-violet-500 shrink-0"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === "pipeline" && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
                            ...
                            <div className="border border-red-950 bg-red-500/[0.02] rounded-2xl p-5 sm:p-6 space-y-4">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-1">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center justify-center gap-2 w-full sm:w-auto px-3 py-2.5 sm:py-2 bg-red-950/40 hover:bg-red-900/20 border border-red-900/60 hover:border-red-500 text-red-400 text-xs font-medium rounded-xl transition shrink-0"
                                    >
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
                            <div className="bg-zinc-900/10 border border-zinc-900 rounded-2xl p-5 sm:p-6 space-y-4">
                                ...
                                <div className="space-y-1.5 pt-2 relative">
                                    <label className="text-[10px] sm:text-[11px] font-mono font-semibold text-zinc-500 uppercase tracking-wider block">
